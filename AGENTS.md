...
## 1.5 製程（**預設工作流，違反即為製程錯誤**）
基礎結構（版面/地基）完成後，**任何功能改動一律照此順序**：

1. 本機 `_v3`（唯一工作 checkout）寫改動。
2. **先在備用站（試用站 `:3080/:3081`）部署並實測**：「本機綠 ≠ 試用站綠」；未在備用站驗證過的東西不得定案。
3. 備用站實測沒問題 → 才 commit、push GitHub。
4. 累積到「一個階段」→ 才把**正式站（`:3000/:3001`）更新成備用站的樣子**。
5. 更新後再從主站推一次 GitHub（確認同步）。
