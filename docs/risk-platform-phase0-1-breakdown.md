...
- `/admin/overview` 管理概覽：報告總數／高風險報告／風險事件／設備指紋統計＋事件 Severity 分布條（CSS，不引第三方圖表）。
...
- 後端：僅 `security_admin` key 可透過 `x-role` 模擬角色預覽遮罩；其他角色以金鑰角色為準。
- `/admin/reports` 新增角色下拉（資安主管／風控分析師／客服），切換後重新載入可見 IP／Raw 遮罩差異。
...
- RBAC 角色與欄位遮罩 ✅
- 審計日誌 API ✅
- 企業 Dashboard（報告列表、事件查詢、設備指紋聚類、管理概覽統計）✅

選配（後續）：真正的圖表庫趨勢圖、客服/分析師專屬登入流程。

已完成（2026-09-06）：

- `/admin/overview` 加入「報告近 7 天趨勢」與「事件近 7 天趨勢」CSS 條狀圖（不引第三方圖表）。

Phase 4 收尾：企業 Dashboard 完整度已達 MVP 標準；第三方圖表庫與專屬登入列為後續選配。
